...
func main() {
	...
	if *startTracker {
		...
		http.HandleFunc("/remove", tracker.HandleRemove)
	...
	// Track active fileID for cleanup
	...
	go func() {
		...
		if activeFileID != "" {
			log.Printf("Stopping sharing of file %s", activeFileID)
			if err := server.StopSharingFile(activeFileID); err != nil {
				log.Printf("Error stopping file share: %v", err)
			}
		}
		os.Exit(0)
	}()
	...
	if *filePath != "" {
		...
		// Get the fileID from metadata
		fileID, err := server.generateFileID(*filePath)
		if err != nil {
			log.Fatalf("failed to generate file ID: %v", err)
		}
		activeFileID = fileID
	}
